...
<style>
    ...
    /* PERFECT MAP CONTAINER - NO SCROLLING */
    .leaflet-container {
        width: 100% !important;
...
with tab_map:
    ...
    st.caption("Signals show news intensity (🟢 Low → 🔴 High) • Click any signal → small popup with headlines • Updates live")
    ...
    if "all_news" in st.session_state and st.session_state.all_news:
        ...
        if location_groups:
            ...
            for data in location_groups.values():
                ...
                # ==================== SMALL POPUP - ONLY HEADLINES + SMALL FONTS ====================
                popup_html = f"""
                <div style="min-width:200px; max-width:260px; font-family: sans-serif; font-size: 11px; line-height: 1.35; padding: 8px;">
                    <strong style="color:#facc15; font-size: 13px;">{data['country']}</strong> 
                    <small style="color:#94a3b8;">({data['count']} stories)</small><br><br>
                    <strong>Headlines:</strong><br>
                    {'<br>• '.join([f"{h[:85]}" for h in data["headlines"][:6]])}
                </div>
                """
                ...
                folium.CircleMarker(
                    ...
                    radius=radius,
                    popup=folium.Popup(popup_html, max_width=260),   # small & clean popup
                    color=color,
                    fill=True,
                    fill_color=color,
                    fill_opacity=0.85,
            ...
            st.caption("🟢 = Low activity 🟡 = Medium 🟠 = High 🔴 = Very High • Click any signal for small headline popup")
        else:
